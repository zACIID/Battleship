...
ChatSchema.methods.addUser = async function (
  ...
  if (!this.users.includes(user_id)) {
    ...
    return Promise.reject(
      new Error('this id is already in the array: ' + user_id)
    );
};

ChatSchema.methods.removeUser = async function (
  user_id: Types.ObjectId
): Promise<IChat> {
...
export async function getChatById(_id: Types.ObjectId): Promise<IChat> {
  var chatData = await Chat.findOne({ _id }).catch((err: Error) =>
